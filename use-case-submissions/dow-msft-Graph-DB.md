...
The equation to calculate M: `M = TE * (TR/EL) * (RR/TR)`
...
`M=1533.12 * (1/35040) * (1/8) = 0.0054kg, 5.47gCO2e`	
 
((Total Embodied Coefficient * (1 hour/4 years of use) * (instance CPU/ platform CPU) )
Note that 4 years of use = 35040 hours (4 * 365 * 24 = 35040)
...
`Total E = 0.082 + .00046 = 0.0824 kwh`

I - 554g (East US & North Central) - Collected on 10/27/2022

`M = 1216.62 * (1/35040) * (2/64)= 0.0012kg, 1.12 gCO2e`	 
(total embodied coefficient * (1 hour/4 years of use) * (instance memory / platform memory))

`M=1216.62 * (1/35040) * (1/8) =  0.0043  ~4.34 gCO2e`	
((total embodied coefficient * (1 hour/4 years of use) * (Instance CPU/ platform CPU) )

`Total M = 1.12 + 4.34 = 5.46 gCO2e`
...
`M = 1533.12 * (1/35040) * (2/64)=  Memory 0.0014, ~1.40  gCO2e`  	
(total embodied coefficient * (1 hour/4 years of use) * (instance memory / platform memory) )

`M = 1533.12 * (1/35040) * (1/8) = CPU 0.0054kg, ~5.47 gCO2e`	
((total embodied coefficient * (1 hour/4 years of use) * (Instance CPU/ platform CPU) )

`Total M = 1.12 + 4.34  = 6.87  gCO2e` 
...
`Total E = 0.164 + .00043 = 0.1644 kgCO2e/kwh`

I - 554 gCO2e (East US & North Central)

`M = 1216.62 * (1/35040) * (2/64)=  Memory 0.0011kg, ~1.12g  gCO2e`	
(total embodied coefficient * (1 hour/4 years of use) * (Instance CPU/ platform CPU) )

`M = 1216.62 * (1/35040) * (1/8) = CPU 0.0043kg  ~4.34 gCO2e`	
(total embodied coefficient * (1 hour/4 years of use) * (Instance CPU/ platform CPU) )
